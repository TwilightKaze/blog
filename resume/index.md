...
- 配置 [eslint][] & [stylelint][] & [prettier][]  以保障代码风格和质量，配置了 [vscode][] 编辑器 `"source.fixAll": true` 配合 [prettier][] 和 [eslint][] 实现保存代码自动格式化, 在提高代码质量的同时提高编码体验.
...
- 针对了apk签名不成功的问题，使用[android studio][]对apk进行签名。
- 编写了README.md文档说明项目启动方式，及需要注意的系列问题，帮助同事快速适应项目。
...
- 对[axios][]请求进行了进一步封装，可针对不同请求场景做处理。
